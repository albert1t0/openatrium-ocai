 ...
 */
function ginkgo_responsive_theme() {
  $items = array();
 ...
 */
function ginkgo_responsive_icon_links(&$links) {
  if (!empty($links)) {
    foreach ($links as $k => $v) {
      ...
      $v['attributes']['class'] .= ' icon-'. _ginkgo_responsive_icon_class($v['href']);
 ...
 */
function ginkgo_responsive_preprocess_page(&$vars) {
  ...
  if (arg(0) === 'admin') {
    ...
    ginkgo_responsive_icon_links($vars['primary_links']);
  }
  ...
  $ie = base_path() . drupal_get_path('theme', 'ginkgo_responsive') .'/ie.css';
 ...
 */
function ginkgo_responsive_preprocess_block(&$vars) {
 ...
 */
function ginkgo_responsive_preprocess_context_block_editable_region(&$vars) {
 ...
 */
function ginkgo_responsive_preprocess_help(&$vars) {
  $vars['layout'] = FALSE;
  $vars['links'] = '';
}
 ...
 */
function ginkgo_responsive_preprocess_node(&$vars) {
  if (!empty($vars['terms'])) {
    $label = t('Tagged');
 ...
 */
function ginkgo_responsive_preprocess_comment(&$vars) {
  // Add a time decay class.
  $decay = _ginkgo_responsive_get_comment_decay($vars['node']->nid, $vars['comment']->timestamp);
 ...
 */
function ginkgo_responsive_preprocess_node_form(&$vars) {
 ...
 */
function ginkgo_responsive_designkit_image($name, $filepath) {
  if ($name === 'logo') {
 ...
 */
function ginkgo_responsive_more_link($url, $title) {
 ...
 */
function ginkgo_responsive_breadcrumb($breadcrumb) {
 ...
 */
function ginkgo_responsive_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
 ...
 */
function ginkgo_responsive_views_mini_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
  $tags[1] = t('Prev');
  $tags[3] = t('Next');
 ...
 */
function ginkgo_responsive_node_preview($node = NULL, $show = FALSE) {
  static $output;
 ...
 */
function ginkgo_responsive_content_multiple_values($element) {
 ...
 */
function ginkgo_responsive_node_submitted($node) {
 ...
 */
function ginkgo_responsive_comment_submitted($comment) {
  $comment->created = $comment->timestamp;
  return ginkgo_responsive_node_submitted($comment);
}
 ...
 */
function ginkgo_responsive_preprocess_views_view_fields(&$vars) {
  foreach ($vars['fields'] as $field) {
    if ($class = _ginkgo_responsive_get_views_field_class($field->handler)) {
      $field->class = $class;
    }
  }
 ...
 */
function ginkgo_responsive_preprocess_views_view_table(&$vars) {
  $view = $vars['view'];
  foreach ($view->field as $field => $handler) {
    if (isset($vars['fields'][$field]) && $class = _ginkgo_responsive_get_views_field_class($handler)) {
 ...
 */
function _ginkgo_responsive_get_views_field_class($handler) {
 ...
 */
function _ginkgo_responsive_get_comment_decay($nid, $timestamp) {
  static $timerange;
 ...
 */
function _ginkgo_responsive_icon_class($path) {
